...
def find_archive_file() -> Path:
    candidates = [
        OUTPUT_DIR / "tg_muztv_archive_period.xlsx",
        OUTPUT_DIR / "tg_muztv_archive_period.csv",
        DATA_DIR / "tg_muztv_archive_raw.xlsx",
        DATA_DIR / "tg_muztv_archive_raw.csv",
    ]
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError("Не найден архив Telegram. Проверь, что collect_archive.py уже отработал.")
...
def main():
    ...
    out_csv = OUTPUT_DIR / "tg_artist_matches.csv"
    out_xlsx = OUTPUT_DIR / "tg_artist_matches.xlsx"
