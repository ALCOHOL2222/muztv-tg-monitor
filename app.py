...
def build_term_regex(term: str):
    ...
    for variant in generate_term_variants(term):
        tokens = re.findall(r"[0-9?-??-???A-Za-z-]+", variant)
        if not tokens:
            continue
        ...
        for token in tokens:
            ...
            # ???????? ?????: ?????? ?????? ?????
            if len(token) <= 3:
                parts.append(rf"(?<![0-9?-??-???A-Za-z_]){re.escape(token)}(?![0-9?-??-???A-Za-z_])")
            else:
                # ????? ??????????: ????? -> ?????? / ???????
                stem_len = max(3, len(token) - 2)
                stem = token[:stem_len]
                parts.append(rf"(?<![0-9?-??-???A-Za-z_]){re.escape(stem)}[?-??-???A-Za-z-]*(?![0-9?-??-???A-Za-z_])")
