...
def try_click_open_in_web(page):
    texts = [
        "Open in Web",
        "OPEN IN WEB",
        "View in Channel",
...
def save_progress_csv(df: pd.DataFrame, path_csv: Path):
...
def main():
    ...
    out_csv = OUTPUT_DIR / "tg_muztv_archive_period_enriched_exact_comments_loggedin_v2.csv"
    out_xlsx = OUTPUT_DIR / "tg_muztv_archive_period_enriched_exact_comments_loggedin_v2.xlsx"

    if out_csv.exists():
        print(f"RESUME_FROM: {out_csv}")
    ...
    with sync_playwright() as p:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                clicked = try_click_open_in_web(page)
                print(f"  clicked = {clicked if clicked else 'NONE'}")
            ...
            if processed_now % 25 == 0:
                save_progress_csv(df, out_csv)
                print(f"AUTOSAVED_CSV at {processed_now} new rows")

        save_progress_csv(df, out_csv)
        print("FINAL_CSV_SAVE_DONE")
